...
app = Flask(__name__)
app.secret_key="1"
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:1@localhost/users'
...
def users_create():
    if request.method == "POST":
        ...
        user=Users(username=username,email=email,password=password,admin=admin)
        db.session.add(user)
        try:
            db.session.commit()
...
def users_update(id):
    ...
    if request.method=="POST":
        ...
        try:
            ...
            return "При обновлении пользоователя произошла ошибка"
    else:
